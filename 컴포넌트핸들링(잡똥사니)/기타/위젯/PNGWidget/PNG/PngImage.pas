...
type
  ...
  public
    ...
    procedure CopyToBitmap32(Bitmap: TBitmap);
...
procedure TPngObject.CopyToBitmap32(Bitmap: TBitmap);
type
  PPngPixels = ^TPngPixels;
  TPngPixels = array[Word] of TRGBTriple;
  PBitmapPixels = ^TBitmapPixels;
  TBitmapPixels = array[Word] of TRGBQuad;
var
  BitmapPixels: PBitmapPixels;
  PngPixels: PPngPixels;
  AlphaPixels: PByteArray;
  Col, Row: Integer;
procedure CopyAlpha255;
  var
    Col, Row: Integer;
  begin
    for Row := 0 to Bitmap.Height - 1 do
     begin
       BitmapPixels := Bitmap.ScanLine[ Row ];
       PngPixels := ScanLine[ Row ];
       for Col := 0 to Bitmap.Width - 1 do
        begin
          BitmapPixels[ Col ].rgbRed := PngPixels[ Col ].rgbtRed;
          BitmapPixels[ Col ].rgbGreen := PngPixels[ Col ].rgbtGreen;
          BitmapPixels[ Col ].rgbBlue := PngPixels[ Col ].rgbtBlue;
          BitmapPixels[ Col ].rgbReserved := 255;
        end;
     end;
  end;
begin
  ...
  case Header.BitmapInfo.bmiHeader.biBitCount of
   24: begin
         ...
         if Header.ImageAlpha <> nil then
          for Row := 0 to Bitmap.Height - 1 do
           begin
             BitmapPixels := Bitmap.ScanLine[ Row ];
             PngPixels := ScanLine[ Row ];
             AlphaPixels := AlphaScanLine[ Row ];

             for Col := 0 to Width - 1 do
              case AlphaPixels[ Col ] of
                0: begin
                     BitmapPixels[ Col ].rgbRed := 0;
                     BitmapPixels[ Col ].rgbGreen := 0;
                     BitmapPixels[ Col ].rgbBlue := 0;
                     BitmapPixels[ Col ].rgbReserved := 0;
                   end;
              255: begin
                     BitmapPixels[ Col ].rgbRed := PngPixels[ Col ].rgbtRed;
                     BitmapPixels[ Col ].rgbGreen := PngPixels[ Col ].rgbtGreen;
                     ...
                     BitmapPixels[ Col ].rgbRed := ( PngPixels[ Col ].rgbtRed * AlphaPixels[ Col ] div $FF ) and $FF;
                     BitmapPixels[ Col ].rgbGreen := ( PngPixels[ Col ].rgbtGreen * AlphaPixels[ Col ] div $FF ) and $FF;
                     BitmapPixels[ Col ].rgbBlue := ( PngPixels[ Col ].rgbtBlue * AlphaPixels[ Col ] div $FF ) and $FF;

                     BitmapPixels[ Col ].rgbReserved := AlphaPixels[ Col ];
                   end;
              end;
